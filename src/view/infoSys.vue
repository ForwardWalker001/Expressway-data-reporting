	...
	<div class="unitInfoContainer">
		<el-card class="box-card">
			...
			<div class="serchBox">
				...
				<el-input placeholder="请输入内容" size="mini" v-model="serchData.personName">
					<template slot="prepend">联系人姓名</template>
				</el-input>
				<el-input placeholder="请输入内容" size="mini" v-model="serchData.stmFullName">
					<template slot="prepend">系统全称</template>
				</el-input>
			...
			<div class="tableContainer">
				...
				<div class="ldrDelBtn">
					...
					<el-button type="success" size="mini" class="addBtn" @click="addUnitInfoDialogVisible = true">新增
					</el-button>
				</div>
				<el-table ref="multipleTable" tooltip-effect="dark" style="width: 100%" :data="tableData"
					...
					<el-table-column label="系统类型" show-overflow-tooltip prop="type">
					</el-table-column>
					<el-table-column label="所属建设单位" show-overflow-tooltip prop="units">
					</el-table-column>
					<el-table-column label="联系人姓名" show-overflow-tooltip prop="personName">
					</el-table-column>
					<el-table-column label="联系人电话" show-overflow-tooltip prop="personPhone">
					</el-table-column>
          ...
          >
			...
			<div class="paginationContainer">
				<el-pagination :current-page="pageInst.currentPage" :page-size="pageInst.pageSize"
					layout="total, prev, pager, next, jumper" :total="pageInst.total"
					@current-change="handleCurrentChange">
				</el-pagination>
			</div>
		</el-card>
		<!-- 新增对话框 -->
		<el-dialog title="新增信息系统信息" :visible.sync="addUnitInfoDialogVisible" width="40%" top="50px">
...
<script>
	export default {
		...
		methods: {
			...
			// 搜索
			serchMethod() {
				this.$axios.post("exMessage/findByCondition",{
						...this.serchData
					})
					.then((res) => {
						if (res.data.success) {
							this.tableData = res.data.data.wrapperList
						} else {
							this.tableData = []
						}
					}).catch((error) => {
